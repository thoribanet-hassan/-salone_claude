...
const fmtMonth = new Intl.DateTimeFormat("ar", { month: "long", year: "numeric", timeZone: "UTC" });
const fmtDay = new Intl.DateTimeFormat("ar", { weekday: "long", day: "numeric", timeZone: "UTC" });
...
export default async function StatsPage({
  ...
  const staffRows = [
    ...
    ...(byStaffMonth.has("unassigned")
      ? [{ id: "unassigned", name: "غير معيّن", isManager: false }]
      : []),
  ];

  return (
    <main className={`${theme.className} min-h-dvh px-4 py-6`}>
      <div className="mx-auto max-w-2xl flex flex-col gap-5">
        <header className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold">📊 إحصائيات العمل</h1>
        ...
        <div className="surface p-3 flex items-center justify-between">
          ...
          {isCurrentMonth ? (
            <span className="px-3 py-1 muted text-sm">الحالي</span>
          ) : (
            <Link href={`/dashboard/stats?month=${shiftMonth(ym, 1)}`} className="px-3 py-1 rounded surface no-underline font-bold">
              التالي ›
            </Link>
        ...
        {isCurrentMonth && (
          <section className="surface p-5">
            <h2 className="font-extrabold mb-3">اليوم — {fmtDay.format(today)}</h2>
                ...
                return (
                  <div key={s.id} className="surface p-3 flex items-center justify-between" style={{ background: "var(--surface-2)" }}>
                    ...
                    <span className="text-sm">
                      أنجز <span className="font-extrabold" style={{ color: "var(--accent)" }}>{nf.format(a.count)}</span>
                      {" · "}حصّل <span className="font-extrabold" style={{ color: "var(--accent)" }}>{nf.format(a.revenue)}</span> ريال
                    </span>
                  </div>
                );
              })}
        ...
        {/* محصلة الشهر لكل فرد */}
        <section className="surface p-5">
          <h2 className="font-extrabold mb-3">محصلة الشهر لكل فرد</h2>
          <table className="w-full text-sm">
            <thead>
              <tr className="muted text-right">
                <th className="py-2 font-bold">الاسم</th>
                <th className="py-2 font-bold">أنجز</th>
                <th className="py-2 font-bold">الإيراد (ريال)</th>
              </tr>
            </thead>
            <tbody>
              {staffRows.map((s) => {
                ...
                return (
                  <tr key={s.id} style={{ borderTop: "1px solid var(--border)" }}>
                    <td className="py-2 font-bold">
                      {s.name}
                      {s.isManager && <span className="muted text-xs"> (المدير)</span>}
                    </td>
              ...
              <tr style={{ borderTop: "2px solid var(--border)" }}>
                <td className="py-2 font-extrabold">الإجمالي</td>
        ...
        {/* التفصيل اليومي */}
        <section className="surface p-5">
          <h2 className="font-extrabold mb-3">التفصيل اليومي</h2>
          {days.length === 0 ? (
            <p className="muted text-sm">لا إنجازات مكتملة في هذا الشهر بعد.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="muted text-right">
                  <th className="py-2 font-bold">اليوم</th>
                  <th className="py-2 font-bold">أُنجز</th>
                  <th className="py-2 font-bold">الإيراد (ريال)</th>
                </tr>
              </thead>
              <tbody>
